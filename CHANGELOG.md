...
- **Snapshot PNG** export now has independent **Title / X-axis label / Y-axis label / Legend** checkboxes (Export section), all on by default. Label inclusion no longer depends on the attached/fixed label mode — labels are drawn into the exported canvas margins either way. The on-map legend (a separate HTML overlay, not part of the SVG points layer) is now rasterized via `html2canvas` and composited onto the exported PNG in its own buffer-zone margin to the right of the plot — the one side with no axis label — so it never overlaps plotted data, with a graceful fallback (legend skipped, status warning shown) if `html2canvas` fails to load. The **Legend** checkbox alone decides inclusion — the panel is captured fully expanded even when minimized on the map — and the per-category color-swatch `<input type=color>` (native OS chrome that `html2canvas` can't rasterize correctly) is excluded from the capture, since the adjacent color swatch already shows the same color as plain, capturable markup.
